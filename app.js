...
app.get('/',async (req,res)=>{
    ...
    if(req.query.edit_id){
        ...
        edit_item = await collection.findOne({_id:ObjectID(edit_id)})
    }
...
app.post('/store_item',async (req,res)=>{
    let database = await dbo.getDatabase();
    const collection = database.collection('items');
    let item = { title: req.body.title, author: req.body.author  };
...
app.post('/update_item/:edit_id',async (req,res)=>{
    ...
    await collection.updateOne({_id:ObjectID(edit_id)},{$set:item});
